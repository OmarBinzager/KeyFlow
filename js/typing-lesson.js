...
let fixedUpDownS = 0.4;
let fixedUpDown = 75 + fixedUpDownS;
let margintop = 0;
let cur_level = 0;
...
function ListLessonsCards(lessons) {
  ...
  let scrollableList = setInterval(() => {
    print("scrolled to " + 401);
    lastAccessedLesson.scrollIntoView({ behavior: "smooth", block: "start" });
    clearInterval(scrollableList);
  }, 100);
}
...
function checkInput(inputLetter) {
  ...
  if (inputLetter === "Tab" || inputLetter === "Enter") {
    ...
    if (localStorage.isArabic === "true" && inputLetter === "Unidentified") {
...
function keyboardToLower() {
  upperKeyboard == true
    ? KeyboardLetters.forEach((l) => (l.innerHTML = l.innerHTML.toLowerCase()))
    : null;
  upperKeyboard = false;
}
function keyboardToUpper() {
  upperKeyboard == false
    ? KeyboardLetters.forEach((l) => (l.innerHTML = l.innerHTML.toUpperCase()))
    : null;
  upperKeyboard = true;
}
